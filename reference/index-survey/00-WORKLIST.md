...
| 15 | SFX3 (patch bank) | none | `InstrumentDefinition` modelled on the raw streams, not the decoded arrays. Sweep all 176 groups. Patch/note-grid tab. | medium | 4 and 14 for audition (soft) |
...
The stronger pattern already exists but is private and duplicated: `ObjectDefinition` and `NPCDefinition` each keep a `_streamRecords` list of (opcode, verbatim payload bytes) and replay it in order, letting only the last occurrence take a fresh payload. Promote that into a shared type plus a base decoder that drives the terminator loop and **throws** on an unknown opcode. Throwing is already this project's deliberate divergence from the client (which silently consumes nothing and desyncs); `FloorOverlayDefinition` and `MapSceneIconDefinition` both do it. Make it the default so 25 new decoders inherit it.
