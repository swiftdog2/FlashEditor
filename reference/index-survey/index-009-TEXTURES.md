...
3. TYPE 12'S OPCODES 2 AND 4 ARE DELIBERATELY SWALLOWED. Texture.cs:518-526 returns true for every opcode on node type 12, with a comment saying two graphs in this cache carry opcodes 2 and 4 that the client's own Node_Sub10_Sub30 does not handle and that read no bytes. They consume nothing, so consumption still balances, but nothing records that they were present - an encoder built from decoded state alone drops them and shortens those two files.
