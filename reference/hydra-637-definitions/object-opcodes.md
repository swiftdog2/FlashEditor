...
2. **Our decoder**, `FlashEditor/Definitions/ObjectDefinition.cs`, method
   `Decode(JagStream, int)` (lines 274-600). Line numbers are as of the commit this document
   was written against; the opcode `case` labels are the durable anchor.
3. **The 639 cache itself**, which proves payload *sizes* empirically and nothing else.

Client and cache are a mismatched pair (client 637, cache 639) - see `AGENTS.md` and the
`README.md` beside this file. Where they disagree the cache wins on sizes and the client wins on
meaning. They did not in fact disagree on a single size; see "Empirical basis" below.
...
Every one of the 56,199 object definitions in the 639 cache was run through three independent
passes: a size-only walk using the payload widths the 637 client implies, a size-only walk using
the widths our decoder uses, and the production `ObjectDefinition.Decode` itself. A pass counts
as clean only when the `0` terminator lands exactly on the last byte of the buffer.

| Pass | Clean | Not exact | Threw |
| --- | --- | --- | --- |
| 637 client widths | 56,199 | 0 | - |
| our widths | 56,199 | 0 | - |
| production `ObjectDefinition.Decode` | 56,199 | 0 | 0 |

**Our decoder's payload widths now match the 637 client's on every opcode, and both consume all
56,199 definitions exactly.** There is no 637-versus-639 width divergence anywhere in this
format, and no remaining size disagreement between the client and our codec. Every disagreement
recorded below is about meaning or field naming, not about bytes.

This was not always true. Opcodes 72 and 75 were mis-sized and desynced several thousand
definitions between them; both are now fixed and both now agree with the client. `README.md` in
this folder tells that story, including why opcode 72 re-synchronised by accident often enough to
slip past an exact-consumption check. The `Cache` column in the table below is the count of
definitions carrying that opcode, taken from the clean walk.
...
| 17 | flag, 0 bytes | `walkable = false`, `actionCount = 0` | 1070, 1465-1467 | flag, 0 bytes | `clipType = 0`, `projectileClipped = false`; `walkable` derived | 4414 | AGREE | The client's `actionCount` is the interact/clip type (op 27 sets it to 1), matching our `clipType`; its `walkable` is the projectile-blocking flag, matching our `projectileClipped`. Our `walkable` is no longer a stored field but a view over the opcode hit map, `!(decoded[17] \|\| decoded[18])` (ObjectDefinition.cs:47-64), which is the same claim the client makes by presence of the flag. |
| 18 | flag, 0 bytes | `walkable = false` | 1071, 1462-1463 | flag, 0 bytes | `projectileClipped = false`; `walkable` derived | 8499 | AGREE | 17 and 18 both clear the same field in the client, as they do in ours. 7 definitions in the cache carry both. |
...
| 24 | `readUnsignedShort()`, then 65535 -> -1 | `anInt2941` | 1452-1457 | `ReadUnsignedShort()`, 65535 kept | `animationId` | 5423 | SEMANTICS-DIFFER | Two bytes either way, so no parse impact. Our `Encode` emits opcode 24 whenever the stream carried it or `animationId != -1` (ObjectDefinition.cs:696), so a stored 65535 round-trips unchanged; the difference is only visible to code reading the field. |
| 27 | flag, 0 bytes | `actionCount = 1` | 1080-1081 | flag, 0 bytes | `clipType = 1` | 10887 | AGREE | |
| 28 | `readUnsignedByte() << 2` | `anInt2966` | 1082, 1449-1450 | `ReadByte() << 2` | `decorDisplacement`, and also `modelBrightness` | 950 | SEMANTICS-DIFFER | Size agrees. `anInt2966` defaults to 64 (line 245) and is consumed by the decor/wall-displacement path in Class305_Sub1.java:953-954, 1065-1066, 1146-1147, 1175, 1205, 1257. It is *not* a lighting value, so the extra `modelBrightness = decorDisplacement` assignment at ObjectDefinition.cs:337 is wrong. The client's brightness is opcode 29, not 28. |
| 29 | `readSignedByte()` | `anInt2931` | 1083-1084 | `ReadSignedByte()` | `ambientLighting`, and also `modelContrast` | 36722 | SEMANTICS-DIFFER | Size agrees. `anInt2931` is the **ambient/brightness** term: line 568 computes `64 + anInt2931` and passes it as the ambient argument at line 684. The `modelContrast = ambientLighting` assignment at ObjectDefinition.cs:338 mislabels it; the contrast term is opcode 39. |
...
| 44 | not handled | - | absent from 1036-1486 | `ReadUnsignedShort()` expanded to a byte array | `unknownArray3` | 0 | CODEC-ONLY | No branch for 44 anywhere in the chain. Absent from the cache too, so it is unreachable either way. |
...
| 72 | `readShort() << 2` | `anInt2946` | 1113, 1409-1410 | `ReadShort() << 2` | `offsetZ` | 371 | AGREE | Formerly read as one unsigned byte, which desynced; corrected at ObjectDefinition.cs:430, with `Encode` writing the short back at :766. `anInt2946` is used interchangeably with `anInt2973`/`anInt2997` in the client at lines 582 and 774, confirming it belongs to the same offset triple as 70 and 71. |
...
| 75 | `readUnsignedByte()` | `anInt2975` | 1116, 1399-1401 | `ReadUnsignedByte()` | `unknownByte75` | 1591 | AGREE | Formerly treated as a bare zero-byte flag, which desynced; corrected at ObjectDefinition.cs:434, with `Encode` writing the byte back at :770. `anInt2975` defaults to -1 (line 254), so it is a value and not a flag. |
...
**No opcode disagrees on size or signedness.** Everything below is meaning or field naming, and
none of it changes a byte on decode or on save.
...
- **Opcodes 78 and 79.** The client's first field differs between the two opcodes (`anInt2996`
  ...
  the cache carry both opcodes; on those, our field holds only the later opcode's value, so the
  editor shows one number where the client has two. The bytes are safe - the encoder replays the
  opcode it did not re-emit from its recorded payload - so this is a display and editing defect,
  not a corruption one. Note that the comment at `ObjectDefinition.cs:795-797` says "forty-odd
  definitions"; the measured count is 81.
...
- **Opcodes 28, 29 and 39.** Widths all agree. The client's brightness/ambient term is opcode 29
  ...
  opcode 28 (ObjectDefinition.cs:337) and `modelContrast` from opcode 29 (:338), so both public
  lighting fields carry the wrong value. Opcode 28's client field is a decor displacement
  (Class305_Sub1.java:953-954), which is what our own `decorDisplacement` name already says. This
  is the one finding here that nothing else in the repository has recorded.
...
inert. Per `README.md`, that is a reason to leave them alone rather than to delete them: the 639
data is silent on them, so matching 637 would be an assumption, not a fix.

Note that 68, 90 and 96 were not on an earlier list of client-absent opcodes; conversely 105,
162, 164, 165, 166, 173 and 177 were on it but *are* handled by the 637 client - they are simply
unused by the 639 data.
...
- **The meaning of opcode 75's byte.** The client stores it in `anInt2975`, default -1
  ...
  unknown; only its width is established, and that is established three times over.
